		...
		for (int j = 0; j < n - i; j++)
		{
			if (j == n - i - 1) // находим позицию первого числа строки
			{
				...
				for (int f = 1; f <= i; f++) //после первого числа дописываем строку из i элементов
				{
					...
					for (int x = 0; x < (i - f); x++)
					{
						num *= (i - x); //это идет вычисление верхнего "урезанного" факториала
						num /= (x + 1);  //это идет вычсиление нижнего одного, оставшегос€ факториала
					}
					// конец вычислени€ элемента
					//ѕочему так? —ократила 2 факториала в фомуле: один исчез, другой стал короче. » чтобы не было переполнени€
					//переменных, сделала умножение и за ним сразу деление. ј чтобы не было целочисленных ошибок при таком делении, 
					//использовала double
					//===============================================
					...
					for (int l = 0; l < (8 - len); l++) //наводим красоту
						cout << " ";
				}
			}
			else
				cout << "    "; // если рисовать до 27 строк, дл€ красоты хватит 4 места на одну позицию
		}
		cout << endl << endl;
	}
#endif // PASCAL_CLEAR
